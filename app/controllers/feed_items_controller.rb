class FeedItemsController < ApplicationController
  def index
    if params[:student_id]
      ...
      @feed_items = FeedItem.where(:user_id => @student.id).order("created_at desc")
      ...
      @feed_items = FeedItem.where(:user_id => @student.id, :course_id => @course.id).order("created_at desc")
      @usage = "#{@student.display_name} in #{@course.name}"
    elsif params[:course_id]
      @course = Course.find(params[:course_id])
      @feed_items = FeedItem.where(:course_id => @course.id).order("created_at desc")
      @usage = @course.name
    else
